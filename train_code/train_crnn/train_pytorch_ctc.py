...
config.train_infofile = [
    '/home/tim/Desktop/ocr.pytorch/Korean dataset dates/Products-Real/evaluation/infofile.txt']
config.val_infofile = '/home/tim/Desktop/ocr.pytorch/Korean dataset dates/Products-Real/evaluation/infofile_test.txt'
config.keep_ratio = True
config.use_log = True
config.pretrained_model = '/home/tim/Desktop/ocr.pytorch/train_code/train_crnn/crnn_models/CRNN-1010.pth'
config.batchSize = 80
config.workers = 10
config.adam = True
# config.lr = 0.00003
...
log_filename = os.path.join(
    'log/', 'loss_acc-'+config.saved_model_prefix + '.log')
...
if not config.random_sample:
    sampler = mydataset.randomSequentialSampler(
...
# setup optimizer
if config.adam:
    optimizer = optim.Adam(crnn.parameters(), lr=config.lr,
...
def val(net, dataset, criterion, max_iter=100):
    ...
    num_correct,  num_all = val_model(
        config.val_infofile, net, True, log_file='compare-'+config.saved_model_prefix+'.log')
...
def trainBatch(net, criterion, optimizer):
    ...
    preds_size = Variable(torch.IntTensor(
        [preds.size(0)] * batch_size))  # seqLength x batchSize
    cost = criterion(preds.log_softmax(2).cpu(), text,
                     preds_size, length) / batch_size
    if torch.isnan(cost):
...
for epoch in range(config.niter):
    ...
    while i < len(train_loader):
        ...
        print('epoch: {} iter: {}/{} Train loss: {:.3f}'.format(epoch,
              i, n_batch, cost.item()))
        loss_avg.add(cost)
        loss_avg.add(cost)
        i += 1
